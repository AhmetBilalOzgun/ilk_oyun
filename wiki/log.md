...
## [2026-09-16] feature | DEV save-sil tuşu + APK telefona kuruldu
- Files changed: `scripts/meta/meta_progress.gd` (`reset_progress()` — user://meta.json sil + `from_dict({})`), `scripts/home.gd` (utility satırına "🗑 SAVE SİL (DEV)" butonu + `_on_wipe_save` onaylı ConfirmationDialog → reset + `reload_current_scene`).
- APK yeniden export (`build/wizard_game.apk`, 56M, debug-signed) + `adb install -r` ile telefona (`to4dzpk7kzmzcadm`) kuruldu (Success) + launcher intent ile başlatıldı (paket `com.example.wizardgame`). Build: `JAVA_HOME=.../openjdk@17 Godot --headless --path . --export-debug "Android" build/wizard_game.apk`. home smoke temiz.

## [2026-09-16] feature | Ritim minigame sektör standardı (Beatstar) + ilk 5 bölüm deneyerek-öğren tutorial
- Files changed: yeniden yazılan `scripts/rpg/rhythm_minigame.gd`; yeni `scripts/meta/tutorial.gd`, `tests/test_tutorial.gd`; düzenlenen `scripts/meta/meta_progress.gd`, `scripts/battle.gd`, `scripts/rpg/run/run_content.gd`, `tests/run_tests.gd`; wiki `hot.md`, `log.md`, `design/turn-based-combat-qte.md`.
- **Ritim yeniden tasarımı (kullanıcı: "çok zor anlaşılıyor" + sektör standardı araştır):** Araştırma → RPG timed-hit soyu (Mario RPG/Paper Mario/Sea of Stars/Mother 3) tek eksen=zamanlama; ritim-şerit soyu (Guitar Hero/Beatstar) notaları SABİT görünür isabet çizgisine akıtır, geniş pencere, ıska komboyu öldürmez. Eski tasarım ikisini de kırıyordu: iki eksen (5-yön jest + zamanlama) aynı anda, sabit referans yok (kapanan halka+fade-in+kuyruk yarışıyor), yanlış yön TÜM komboyu öldürüyor. Karar (AskUserQuestion): **Beatstar tarzı** — sabit dikey isabet çizgisi + GOOD/PERFECT bandı, notalar sağdan sola akar (`x = _hit_x + (t-_clock)*_speed`), her nota kendi ikonunu (●/◀▶▲▼) taşır, pencereler genişletildi (PERFECT 0.13→0.16, GOOD 0.30→0.36), **FAIL-SOFT per-nota**: `_break_combo` KALDIRILDI; yanlış yön/kaçan pencere yalnız o notayı düşürür (`_miss_note`), kalanlar akar. `broke = _missed*2 > tiles`. Motor sözleşmesi (setup/tile_resolved/finished) DEĞİŞMEDİ.
- **İlk 5 bölüm tutorial (kullanıcı: durduran text-box değil, deneyerek öğren):** Karar (AskUserQuestion): kısa bağlamsal ipuçları + telegraf (modal yok). Yeni saf `Tutorial` (`scripts/meta/tutorial.gd`): `TUTORIAL_LEVELS=5`, `is_tutorial`, `rhythm_caps(level)` = {combo_len, speed_cap} kademeli tavan (L1 tek yavaş nota→L5 serbeste yakın), `hint(key)` metin tablosu. `battle._tutorial_hint(key)` = akıp giden tek satır `_flash`, ömür boyu bir kez (`Meta.seen_hints`+`has_seen_hint`/`mark_hint_seen`, kalıcı). İpuçları: rhythm/swipe/perfect/charge/orb/cards/build. `_on_input_requested` tutorial'da `combo_len`+`speed_scale`'i `rhythm_caps` ile tavanlar (yalnız kolaylaştırır). `run_content`: tutorial eşiği `i<3`→`i<TUTORIAL_LEVELS`; L1-L2 tek zayıf düşman, L3-L5 nazik 2'li; ELITE/heal/hazine ve dallanma yalnız tutorial sonrası. `is_build_level` DEĞİŞMEDİ (build hâlâ i=3=L4).
- Test **2092 geçti, 0 kaldı** (+17). battle.tscn headless smoke temiz. Yeni class_name `Tutorial` için global cache `--editor --quit` ile yenilendi. **Telefonda elle doğrulanmalı:** L1 tek yavaş nota okunur mu, çizgi hizası net mi, yanlış yön komboyu öldürmüyor mu, L1-5 ipuçları bir kez görünüp susuyor mu. → [[Turn-Based Savaş ve QTE]]

## [2026-09-16] feature | Makro kapanış: run puanı, keşif/kodeks, günlük-haftalık meydan + iki iptal
- Files changed: yeni `scripts/rpg/run/run_score.gd`, `scripts/meta/challenge.gd`, `scripts/meta/codex_data.gd`, `scripts/codex.gd`, `scenes/codex.tscn`, `tests/test_macro.gd`; düzenlenen `scripts/meta/meta_progress.gd`, `scripts/rpg/run/{run_state,run_manager}.gd`, `scripts/{battle,home}.gd`, `tests/run_tests.gd`; wiki `design/macro-game-roadmap.md`, `hot.md`, `log.md`.
- **Numeric run-end score**: `RunScore.compute` (saf; düğüm/kat + altın + relik + arketip + elit/boss + zafer). `RunState`'e `battles_won/elites_won/bosses_won` sayaçları (RunManager artırır). `Meta.best_score`/`record_score`; özet panelde "★ PUAN … (rekor …)".
- **Discovery + Kodeks**: `Meta.discovered`/`discover(key)` kalıcı; battle form/arketip/relik/düşman ilk karşılaşmada işler. `CodexData` (RunContent türevi tek kaynak) → `scenes/codex.tscn` keşfedilen=isim+açıklama, değilse 🔒 ???. Home "📖 KODEKS".
- **Günlük/haftalık meydan (Faz 4 MVP, sunucusuz)**: `Challenge.daily_seed`/`weekly_seed` tarihten deterministik (aynı gün herkes aynı harita); seed'li `map_rng`; `Meta.daily_best`/`weekly_best`. Async sosyal STUB `Challenge.percentile` (lojistik) → "oyuncuların %N'ini geçtin". Gerçek sunucu leaderboard hâlâ açık.
- **İki KALICI İPTAL (kullanıcı kararı 2026-09-16)**: orb push-your-luck ve elit "build sınayan özel kural" yapılmayacak — roadmap'te işaretlendi. Ayrıca wiki hatası düzeltildi: Plazma arketip havuzu zaten koddaydı (`RunContent.plasma_archetypes`), "başlamadı" yanlıştı.
- Doğrulama: **2075 kontrol geçti, 0 başarısız** (Godot 4.7.2 headless). Editor import pass: 0 script hatası (battle/home/codex derleniyor). Telefonda elle UI doğrulaması (kodeks/meydan özet ekranı/percentile) yapılmadı; challenge zorluk sabiti (CHALLENGE_LEVEL=9) ve percentile REF/SPREAD tune edilecek.
